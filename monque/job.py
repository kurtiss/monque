...
class MonqueJob(object):
	...
	def __deserialize__(cls, message):
		...
		if job_cls_path:
			job_cls = util.full_import(job_cls_path)
			
			if job_cls != cls:
				return job_cls.__deserialize__(message)
		
		func = util.full_import(message['func'])
	...
	def __serialize__(self):
		serialized = dict(
			id			= self.id,
			func		= "{0.__module__}.{0.__name__}".format(self._func),
		...
		if self.__class__ != MonqueJob:
			serialized['cls'] = "{0.__module__}.{0.__name__}".format(self.__class__)
